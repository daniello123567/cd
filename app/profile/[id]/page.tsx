...
return (
    ...
    <div className={`h-full pt-[4rem] dark:dark:[--pattern-foreground:var(--color-white)]/5 bg-size-[10px_10px] dark:bg-white/0.75 flex flex-col py-[.75rem]  bg-[radial-gradient(var(--pattern-foreground)_1px,transparent_0)]  min-h-[5rem] dark:border-[#3F3E47]   mx-auto md:max-w-[598px] border-[#EDEDF5] border-l border-r`}>
...
{isPending?<div className='h-[5em] w-full flex justify-center items-center'>
  ...
  {isPending?<Loading/>:userposts?.map((post)=>{
    return <Singletweet key={post.id} media={post.media} user_id={post.user_id} user_image={post.user_image} post={post.post} Name={post.Name} username={post.username} created_at={post.created_at} id={post.id}/>
  })}
</div>}
